...
class Database(sqlite3.Connection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs["check_same_thread"] = False  # The thread safety is handled by the CustomLock class
    ...
    def get_table(self, table_name: str) -> DynamicTable:
        ...
        if table_name in self.tables:
            ...
            result = self.get(f"SELECT * FROM {table_name} LIMIT 1")  # Verify that the table exists
            if result:
                self.tables[table_name] = DynamicTable(table_name, self)
                return self.tables[table_name]
            else:
