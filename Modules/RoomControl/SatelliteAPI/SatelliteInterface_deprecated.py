...
class Satellite:
    ...
    async def _downlink_event(self, object_ref, event_name, *args, **kwargs):
        ...
        try:
            ...
            logging.warning(f"{self.name} returned an invalid response to an event message: {e}")
        except Exception as e:
            logging.error(f"Error sending event to {self.name}: {e}")
            logging.exception(e)


class SatelliteInterfaceDeprecated(RoomModule):
    is_webserver = True
    ...
    async def uplink_event(self, request):
        ...
        """
        logging.debug("Received uplink event")
        payload = await request.json()
        for satellite in self.satellites.values():
            if satellite.name == payload["name"]:
